...
const companyId = await page.evaluate(async () => {
  ...
  const dash = await fetch("/api/dashboard?companyId=" + cid, { headers: { Authorization: "Bearer " + t } });
  return { meOk: !!cid, dashStatus: dash.status, dashBody: (await dash.text()).slice(0, 300) };
});
console.log(JSON.stringify(companyId, null, 1));
await browser.close();
